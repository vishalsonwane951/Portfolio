...
const globalStyles = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  ::-webkit-scrollbar{width:4px}::-webkit-scrollbar-track{background:var(--bg)}::-webkit-scrollbar-thumb{background:var(--p1);border-radius:2px}
  ::selection{background:var(--p1);color:#fff}
  .admin-wrap{display:flex;min-height:100vh}
  .sidebar{width:240px;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;position:fixed;top:0;bottom:0;left:0;z-index:100;transition:transform 0.3s}
  .sidebar-header{padding:1.5rem 1.25rem;border-bottom:1px solid var(--border)}
  .brand{display:flex;align-items:center;gap:10px}
  .brand-orb{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,var(--p1),#5b4fcf);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;color:#fff;box-shadow:0 0 16px var(--glow);flex-shrink:0}
  ...
  .nav-section-label{font-size:10px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:0.12em;font-family:var(--mono);padding:0.75rem 0.5rem 0.25rem}
  .nav-item{display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:9px;cursor:pointer;transition:all 0.2s;font-size:13px;font-weight:500;color:var(--muted);border:none;background:none;width:100%;text-align:left}
  .nav-item:hover{color:var(--text);background:var(--surface2)}
  .nav-item.active{color:var(--p2);background:rgba(124,111,247,0.12);border:1px solid rgba(124,111,247,0.2)}
  .nav-icon{font-size:16px;flex-shrink:0;width:20px;text-align:center}
  .nav-badge{margin-left:auto;font-size:10px;padding:2px 7px;border-radius:10px;background:rgba(248,113,113,0.15);color:var(--red);border:1px solid rgba(248,113,113,0.25);font-weight:700}
  .sidebar-footer{padding:1rem;border-top:1px solid var(--border)}
  .logout-btn{display:flex;align-items:center;gap:8px;padding:9px 12px;border-radius:9px;cursor:pointer;font-size:13px;font-weight:500;color:var(--red);border:1px solid rgba(248,113,113,0.2);background:rgba(248,113,113,0.05);width:100%;transition:all 0.2s}
  .logout-btn:hover{background:rgba(248,113,113,0.12)}
  .main{margin-left:240px;flex:1;min-height:100vh;display:flex;flex-direction:column}
  .topbar{padding:0 2rem;height:60px;background:rgba(10,10,15,0.8);backdrop-filter:blur(20px);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:50}
  .topbar-title{font-size:16px;font-weight:700;color:var(--text)}
  .topbar-right{display:flex;align-items:center;gap:12px}
  .admin-chip{font-size:11px;padding:4px 12px;border-radius:20px;background:rgba(124,111,247,0.1);border:1px solid rgba(124,111,247,0.2);color:var(--p2);font-family:var(--mono)}
  ...
  .stat-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:1.25rem;position:relative;overflow:hidden}
  .stat-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--p1),var(--p3))}
  ...
  .stat-icon{position:absolute;right:1rem;top:50%;transform:translateY(-50%);font-size:2rem;opacity:0.2}
  .form-grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem}
  .form-group{display:flex;flex-direction:column;gap:5px;margin-bottom:0.75rem}
  .form-group.full{grid-column:1/-1}
  .form-label{font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.1em;font-family:var(--mono)}
  .form-input{padding:9px 13px;border-radius:9px;font-size:13.5px;background:var(--surface2);border:1px solid var(--border2);color:var(--text);font-family:var(--font);outline:none;transition:all 0.2s;width:100%}
  .form-input:focus{border-color:var(--p1);box-shadow:0 0 0 3px rgba(124,111,247,0.12)}
  textarea.form-input{resize:vertical;min-height:90px}
  .form-input::placeholder{color:var(--muted2)}
  .btn{padding:8px 18px;border-radius:9px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;border:none;display:inline-flex;align-items:center;gap:7px;font-family:var(--font)}
  .btn-primary{background:linear-gradient(135deg,var(--p1),#5b4fcf);color:#fff;box-shadow:0 4px 16px var(--glow)}
  ...
  .btn-success{background:rgba(6,214,160,0.1);color:var(--accent);border:1px solid rgba(6,214,160,0.25)}
  .btn-success:hover{background:rgba(6,214,160,0.2)}
  .table-wrap{overflow-x:auto}
  table{width:100%;border-collapse:collapse}
  th{text-align:left;font-size:11px;font-weight:700;color:var(--muted2);text-transform:uppercase;letter-spacing:0.1em;padding:10px 14px;border-bottom:1px solid var(--border);font-family:var(--mono)}
  td{padding:12px 14px;border-bottom:1px solid var(--border);font-size:13px;color:var(--muted);vertical-align:middle}
  tr:last-child td{border-bottom:none}
  tr:hover td{background:rgba(255,255,255,0.02)}
  .td-title{color:var(--text);font-weight:600}
  .badge{display:inline-block;font-size:10px;padding:2px 9px;border-radius:20px;font-weight:700;letter-spacing:0.04em}
  .badge-green{background:rgba(6,214,160,0.1);color:var(--accent);border:1px solid rgba(6,214,160,0.2)}
  .badge-gold{background:rgba(251,191,36,0.1);color:var(--gold);border:1px solid rgba(251,191,36,0.2)}
  .badge-purple{background:rgba(124,111,247,0.1);color:var(--p2);border:1px solid rgba(124,111,247,0.2)}
  .badge-red{background:rgba(248,113,113,0.1);color:var(--red);border:1px solid rgba(248,113,113,0.2)}
  .tags-wrap{display:flex;flex-wrap:wrap;gap:6px;padding:8px;background:var(--surface2);border:1px solid var(--border2);border-radius:9px;min-height:44px;cursor:text;transition:all 0.2s}
  .tags-wrap:focus-within{border-color:var(--p1);box-shadow:0 0 0 3px rgba(124,111,247,0.12)}
  .tag-chip{display:flex;align-items:center;gap:5px;font-size:11.5px;padding:3px 9px;border-radius:6px;background:rgba(124,111,247,0.1);border:1px solid rgba(124,111,247,0.2);color:var(--p2)}
  .tag-chip button{background:none;border:none;cursor:pointer;color:var(--p2);font-size:13px;padding:0;line-height:1}
  .tags-input{border:none;background:none;outline:none;font-size:13px;color:var(--text);font-family:var(--font);min-width:100px;flex:1;padding:2px 4px}
  .tags-input::placeholder{color:var(--muted2)}
  ...
  .modal{background:var(--surface);border:1px solid var(--border2);border-radius:16px;padding:2rem;width:100%;max-width:640px;max-height:90vh;overflow-y:auto;position:relative}
  .modal-title{font-size:17px;font-weight:700;color:var(--text);margin-bottom:1.5rem;padding-bottom:0.75rem;border-bottom:1px solid var(--border)}
  .modal-close{position:absolute;top:1rem;right:1rem;background:none;border:none;color:var(--muted);cursor:pointer;font-size:20px;padding:4px;transition:color 0.2s}
  .modal-close:hover{color:var(--text)}
  .msg-card{background:var(--surface2);border:1px solid var(--border);border-radius:10px;padding:1rem 1.25rem;margin-bottom:0.75rem;transition:all 0.2s}
  .msg-card.unread{border-left:3px solid var(--p1)}
  ...
  .msg-email{font-size:12px;color:var(--p2)}
  .msg-time{font-size:11px;color:var(--muted2);font-family:var(--mono);margin-left:auto}
  .msg-body{font-size:13px;color:var(--muted);line-height:1.7;white-space:pre-wrap}
  ...
  .login-bg{position:absolute;inset:0;background:radial-gradient(ellipse 60% 60% at 50% 50%,rgba(124,111,247,0.1) 0%,transparent 70%)}
  .login-card{background:var(--surface);border:1px solid var(--border2);border-radius:20px;padding:2.5rem;width:100%;max-width:400px;position:relative;z-index:1}
  .login-title{font-size:22px;font-weight:800;text-align:center;margin-bottom:4px;background:linear-gradient(135deg,var(--p1),var(--p3));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
  .login-sub{font-size:13px;color:var(--muted);text-align:center;margin-bottom:2rem}
  .login-err{background:rgba(248,113,113,0.1);border:1px solid rgba(248,113,113,0.25);color:var(--red);padding:10px 14px;border-radius:9px;font-size:13px;margin-bottom:1rem}
  .toast-wrap{position:fixed;bottom:1.5rem;right:1.5rem;z-index:9999;display:flex;flex-direction:column;gap:8px}
  .toast{padding:12px 18px;border-radius:10px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:8px;animation:slideIn 0.3s ease;max-width:320px;box-shadow:0 8px 32px rgba(0,0,0,0.4)}
  .toast-success{background:rgba(6,214,160,0.15);border:1px solid rgba(6,214,160,0.3);color:var(--accent)}
        ...
        <div className="login-page">
            ...
            <div className="login-card">
                <div style={{ textAlign: "center", marginBottom: "1.5rem" }}>
                    <div
                        className="brand-orb"
                        style={{ width: 52, height: 52, fontSize: 18, margin: "0 auto 1rem", borderRadius: 14 }}
                    >
                        VS
                    </div>
                    <div className="login-title">Admin Panel</div>
                    <div className="login-sub">Portfolio CMS · Sign in to manage content</div>
                </div>
...
function Dashboard({ token, unreadCount }) {
    const [stats, setStats] = useState({ projects: 0, skills: 0, education: 0, messages: 0, unread: 0 });
    ...
    return (
        <div>
            ...
            {unreadCount > 0 && (
                <div className="card" style={{ borderColor: "rgba(124,111,247,0.3)", background: "rgba(124,111,247,0.05)" }}>
            ...
            <div className="card">
                <div className="card-title" style={{ marginBottom: "1rem" }}>📋 Quick Guide</div>
                {[
                    ...
                    ["📬 Messages", "Read & manage contact form submissions (email sent automatically)"],
                ].map(([title, desc]) => (
                    <div key={title} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: "1px solid var(--border)" }}>
                        <div style={{ fontWeight: 700, color: "var(--p2)", minWidth: 120, fontSize: 13 }}>{title}</div>
    ...
    useEffect(() => {
        api("/portfolio/hero", {}, token)
            .then((d) =>
                setForm(
                    d.data || {
                        name: "", tagline: "", subtitle: "", role: "", location: "",
                        available: true, photoUrl: "", company: "", companyRole: "",
                        companySince: "", techChips: [],
                        stats: [
        ...
        <form onSubmit={save}>
            <div className="card">
                ...
                <div className="form-grid">
                    ...
                    <div className="form-group">
                        <label className="form-label">Since (e.g. "2024 – Present · Pune")</label>
                        <input className="form-input" value={form?.companySince || ""} onChange={(e) => set("companySince", e.target.value)} />
                    </div>
                </div>
                <div className="form-group">
                    <label className="form-label">Tech Chips (shown on card)</label>
                    <TagsInput value={form?.techChips || []} onChange={(v) => set("techChips", v)} placeholder="MERN, Python, AWS..." />
                </div>
                <div className="sep" />
                <div style={{ fontWeight: 700, color: "var(--text)", marginBottom: "0.75rem", fontSize: 14 }}>Stats (Hero Section)</div>
                <div className="form-grid">
                    {(form?.stats || []).map((s, i) => (
                ...
                <div style={{ marginTop: "1.5rem", display: "flex", gap: 10 }}>
                    <button type="submit" className="btn btn-primary" disabled={saving}>
                        {saving ? <><div className="spinner" /> Saving...</> : "💾 Save Hero Section"}
                    </button>
                </div>
            </div>
        </form>
        ...
        <form onSubmit={save}>
            <div className="card">
                ...
                <div className="form-grid">
                    ...
                    <div className="form-group">
                        <label className="form-label">Location</label>
                        <input className="form-input" value={form?.locationDetail || ""} onChange={(e) => set("locationDetail", e.target.value)} />
                    </div>
                    <div className="form-group">
                        <label className="form-label">Languages</label>
                        <input className="form-input" value={form?.languages || ""} onChange={(e) => set("languages", e.target.value)} />
                    </div>
                    <div className="form-group">
                    ...
                    <div className="form-group">
                        ...
                        <input className="form-input" value={form?.status || ""} onChange={(e) => set("status", e.target.value)} placeholder="Open to Work" />
                    </div>
                </div>
                <div className="sep" />
                ...
                <div style={{ marginTop: "1.5rem" }}>
                    <button type="submit" className="btn btn-primary" disabled={saving}>
                        {saving ? <><div className="spinner" /> Saving...</> : "💾 Save About Section"}
                    </button>
                </div>
            </div>
        </form>
...
function SkillsManager({ token, toast }) {
    ...
    const openAdd = () => setModal({ mode: "add", data: { title: "", icon: "", skills: [], order: cats.length } });
    ...
    const save = async () => {
        ...
        try {
            if (modal.mode === "add") {
                await api("/portfolio/skills", { method: "POST", body: JSON.stringify(modal.data) }, token);
                toast("Skill category added!", "success");
            } else {
                await api(`/portfolio/skills/${modal.data._id}`, { method: "PUT", body: JSON.stringify(modal.data) }, token);
                toast("Skill category updated!", "success");
            }
            setModal(null);
            load();
        } catch (err) {
    ...
    const del = async (id) => {
        ...
        try {
            await api(`/portfolio/skills/${id}`, { method: "DELETE" }, token);
        ...
        <div>
            <div className="card">
                ...
                {loading ? (
                    ...
                    <div className="empty"><div className="empty-icon">⚙️</div>No skill categories yet</div>
                ) : (
                ...
                <div className="modal-overlay" onClick={() => setModal(null)}>
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-title">
                            {modal.mode === "add" ? "Add Skill Category" : "Edit Skill Category"}
                        </div>
                        <div className="form-grid">
                            <div className="form-group">
...
function ProjectsManager({ token, toast }) {
    ...
    const blankProject = () => ({
        title: "", description: "", skills: [], keyFeatures: [], progress: 100,
        status: "Completed", projectLink: "", githubLink: [], featured: false,
        icon: "💻", order: projects.length,
    });
    ...
    const save = async () => {
        ...
        try {
            if (modal.mode === "add")
                await api("/portfolio/projects", { method: "POST", body: JSON.stringify(modal.data) }, token);
            else
                await api(`/portfolio/projects/${modal.data._id}`, { method: "PUT", body: JSON.stringify(modal.data) }, token);
    ...
    const del = async (id) => {
        ...
        try {
            await api(`/portfolio/projects/${id}`, { method: "DELETE" }, token);
            toast("Deleted!", "success");
            load();
        } catch (err) {
            toast(err.message, "error");
        ...
        <div>
            <div className="card">
                <div className="card-header">
                    ...
                    <button className="btn btn-primary" onClick={() => setModal({ mode: "add", data: blankProject() })}>
                        + Add Project
                    </button>
                </div>
                {loading ? (
                            ...
                            <tbody>
                                {projects.map((p) => (
                                    <tr key={p._id}>
                                        ...
                                        <td>
                                            <span className={`badge ${p.progress === 100 ? "badge-green" : "badge-gold"}`}>
                                                {p.status}
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div style={{ display: "flex", gap: 6 }}>
                                                <button className="btn btn-ghost btn-sm" onClick={() => setModal({ mode: "edit", data: { ...p } })}>✏️ Edit</button>
...
function EducationManager({ token, toast }) {
    ...
    const blank = () => ({
        type: "degree", dotStyle: "deg", icon: "🎓", yearRange: "", title: "",
        institution: "", location: "", employmentType: "", cgpa: "", chips: [],
        isCurrent: false, certFile: "", order: items.length,
    });
    ...
    const save = async () => {
        ...
        try {
            if (modal.mode === "add")
                await api("/portfolio/education", { method: "POST", body: JSON.stringify(modal.data) }, token);
            else
                await api(`/portfolio/education/${modal.data._id}`, { method: "PUT", body: JSON.stringify(modal.data) }, token);
            toast("Saved!", "success");
            setModal(null);
            load();
        } catch (err) {
    ...
    const del = async (id) => {
        ...
        try {
            await api(`/portfolio/education/${id}`, { method: "DELETE" }, token);
            toast("Deleted!", "success");
            load();
        } catch (err) {
            toast(err.message, "error");
    ...
    const setField = (k, v) => setModal((m) => ({ ...m, data: { ...m.data, [k]: v } }));
    const typeIconMap = { experience: "💼", degree: "🎓", certification: "⚡" };
        ...
        <div>
            <div className="card">
                <div className="card-header">
                    ...
                    <button className="btn btn-primary" onClick={() => setModal({ mode: "add", data: blank() })}>
                        + Add Entry
                    </button>
                </div>
                {loading ? (
                    ...
                    <div className="empty"><div className="empty-icon">🎓</div>No entries yet</div>
                ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                        {items.map((item) => (
                            <div key={item._id} style={{ background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 10, padding: "1rem 1.25rem", display: "flex", alignItems: "flex-start", gap: 14 }}>
                                <div style={{ fontSize: 24, paddingTop: 2 }}>{item.icon || typeIconMap[item.type]}</div>
                                <div style={{ flex: 1 }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                                        ...
                                        <span className={`badge badge-${item.type === "experience" ? "green" : item.type === "degree" ? "gold" : "purple"}`}>
                                            {item.type}
                                        </span>
                                    </div>
            ...
            {/* ── EDUCATION MODAL (FIXED) ── */}
            {modal && (
                <div className="modal-overlay" onClick={() => setModal(null)}>
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        <button className="modal-close" onClick={() => setModal(null)}>×</button>
                        <div className="modal-title">
                            {modal.mode === "add" ? "Add Entry" : "Edit Entry"}
                        </div>
                        <div className="form-grid">
                            <div className="form-group">
...
function Messages({ token, toast, onUnreadChange }) {
    ...
    const load = useCallback(() => {
        ...
        api(`/contact${q}`, {}, token)
            .then((d) => {
                setMsgs(d.data || []);
                if (filter === "all") {
                    const unread = (d.data || []).filter((m) => !m.isRead).length;
    ...
    const markRead = async (id) => {
        try {
            ...
            setMsgs((ms) => ms.map((m) => (m._id === id ? { ...m, isRead: true } : m)));
            onUnreadChange(msgs.filter((m) => !m.isRead && m._id !== id).length);
        } catch (err) {
            toast(err.message, "error");
        }
    };
        ...
        <div>
            <div className="card">
                ...
                {loading ? (
                    ...
                    <div className="empty">
                        <div className="empty-icon">📭</div>No messages{filter === "unread" ? " unread" : ""}
                    </div>
                ) : (
                    msgs.map((m) => (
                        <div key={m._id} className={`msg-card ${!m.isRead ? "unread" : ""}`}>
                            ...
                            <div className="msg-actions">
                                <a href={`mailto:${m.email}?subject=Re: Portfolio Contact`} className="btn btn-ghost btn-sm">✉️ Reply</a>
                                {!m.isRead && (
                                    <button className="btn btn-success btn-sm" onClick={() => markRead(m._id)}>✓ Mark Read</button>
                                )}
                                <button className="btn btn-danger btn-sm" onClick={() => del(m._id)}>🗑️ Delete</button>
                            </div>
                        </div>
                    ))
                )}
...
export default function VSAdminPanel() {
    ...
    const handleLogin = (t) => {
        localStorage.setItem("vs_admin_token", t);
        setToken(t);
    };
    const handleLogout = () => {
    ...
    const sectionTitles = {
        ...
        skills: "Skills", projects: "Projects", education: "Education & Experience",
